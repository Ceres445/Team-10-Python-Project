...
from apps.public_api.models import Category
...
def authenticated_home(user, model, **kwargs):
    return model.objects.all().exclude(
        **{kwargs.get("name"): "Class"}
    ) | model.objects.all().filter(
        **{
            kwargs.get(
                "class_in"
            ): user.profile.courses.all(),  # include classes in which user is in
            kwargs.get("name"): "Class",
        }
    )  # return class objects for user
...
def authenticated_classes(user, model, **kwarg):
    return model.objects.all().filter(
        **{kwarg["courses"]: user.profile.courses}
    ...
    if model is not Category:
        if category_param in ["Class", "Public", "Forum", "Site"]:
            ...
            queryset = queryset.filter(
                **{kwargs.get("name"): category_param.strip("'")}
            )  # filter against given category
            # print(queryset)
            if category_param == "Class":
                if user.is_authenticated:
                    if not user.is_staff:
                        queryset = queryset.filter(
                            **{kwargs.get("class_in"): user.profile.courses.all()}
                        )
                else:
