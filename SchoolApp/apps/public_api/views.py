...
class PostList(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        )  # TODO: block users from creating posts in wrong categories
                ...
                if post.category.name == "Class":
                    if self.request.user.is_authenticated:
                        ...
                        raise PermissionDenied(
                            "Non authenticated users cannot see classes"
                        )
        return queryset
...
class CategoryList(generics.ListAPIView):
    def get_queryset(self):
        queryset = filter_queryset(
            ...
            Category,
            {"authenticated": authenticated_home, "anon": anon_home},
            class_in="key_class__in",
            name="name",
        )
        queryset = parse_args(
            queryset, self.request.user, Category, class_in="key_class__in", name="name"
        )
...
class CategoryDetail(generics.RetrieveUpdateDestroyAPIView):
    ...
    ]  # only admin users can create new categories manually
...
class AssignmentList(generics.ListAPIView):
    ...
    def get_queryset(self):
        return filter_queryset(
            self.request.user,
            Assignment,
            {"authenticated": authenticated_classes, "anon": anon},
...
class UploadList(generics.ListAPIView):
    ...
    def get_queryset(self):
        return filter_queryset(
            ...
            {"authenticated": authenticated_classes, "anon": anon},
            **{
                "courses": "assignment_key_class__in",
                "teacher": "assignment_key_class__teacher_id",
            }
        )
